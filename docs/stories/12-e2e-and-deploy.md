...
## 1. Goal
Verify the full happy path in a real browser, fix any integration gaps that Phase 1 stories left at their seams, and confirm the GitHub Pages deploy serves the production bundle correctly.
...
- Workflow: `.github/workflows/deploy.yml` from S00.
...
- One end-to-end smoke test in `e2e/smoke.spec.ts` using **Playwright** (add as the only new devDependency). The test runs against `npm run preview` and walks:
  ...
  3. Land on Practice. Use a mocked AI response (intercepted via `page.route` against `api.deepseek.com`) returning a canned `TutorResponse` JSON payload in OpenAI Chat Completions shape.
...
- A README section: "Running locally" + "Deploying to GitHub Pages" (enable Pages, set repo name in `vite.config.ts`).
...
3. Wire the test into CI: a new job in `.github/workflows/deploy.yml` that runs before the deploy step (`needs:` dependency). The deploy step only runs if the smoke test passes.
4. Manual checklist run (document in a PR description, not a file):
   - Visit the deployed URL.
   - Walk through with a real DeepSeek key (developer's own).
   - Confirm: BYOK saves; practice turn returns; checkpoint advances; export+reimport restores state.
...
- `.github/workflows/deploy.yml` (extend with the e2e job)
...
- Deployment to GitHub Pages succeeds on push to `main`.
