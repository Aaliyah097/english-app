# S00 — Project scaffolding & GitHub Pages CI
...
## 1. Goal
Stand up a Vite + React + TypeScript project that builds, lints, tests, and deploys to GitHub Pages on push to `main`. No product code beyond a "hello" route that renders the warm off-white background — just enough to prove the pipeline works end-to-end.

## 2. Context (read these first)
- Plan: [`docs/PLAN.md`](../PLAN.md) §2 (Tech stack) and §1 (why GH Pages = static = BYOK).
...
3. Add dependencies: `react`, `react-dom`, `zod`, `openai` (used against DeepSeek's OpenAI-compatible endpoint — see S04).
...
13. Add `.github/workflows/deploy.yml` that on push to `main`: install, typecheck, lint, test, build, upload artifact, deploy via `actions/deploy-pages@v4`.
...
- Pushing to `main` triggers the workflow and publishes to GitHub Pages.
...
npm run test -- --run
npm run build
# Then push to a test branch and watch the Pages workflow succeed,
...
- GitHub Pages must be enabled in repo settings → Pages → Source: "GitHub Actions". The workflow can run but won't publish until that toggle is on; flag this in the README.
